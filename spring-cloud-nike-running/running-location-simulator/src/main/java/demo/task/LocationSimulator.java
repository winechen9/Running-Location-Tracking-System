...
public class LocationSimulator implements Runnable {
    ...
    private Integer reportInterval = 500; //500ms
        ...
        try {
            ...
            while (!Thread.interrupted()){
                ...
                if (currentPosition != null) {
                    if (shouldMove){
                        moveRunningLocation();
                        currentPosition.setSpeed(speedInMaps);
                    }else {
                    ...
                    final CurrentPosition currentPosition = new CurrentPosition(this.currentPosition.getRunningId(),
                            new Point(this.currentPosition.getPosition().getLatitude(), this.currentPosition.getPosition().getLongitude(),
                                    this.currentPosition.getRunnerStatus(), this.currentPosition.getSpeed(), this.currentPosition.getLeg().getHeading(),
                                    medicalInfoToUse));
    ...
    // set new positions of running locations based on current position and speed
    private void moveRunningLocation(){
        double distance = speedInMaps * reportInterval / 1000.0; // new added distance this time
        ...
        for (int i = currentPosition.getLeg().getId(); i < legs.size(); i++){
            ...
            if (Double.doubleToRawLongBits(excess) == 0) {
                ...
                //use the new position calculation method in NavUtils class
        ...
        setStartPosition(); //reset
    }

    // position running location at start of the path
    public void setStartPosition(){
        currentPosition = new PositionInfo();
        currentPosition.setRunningId(this.runningId);
        Leg leg = legs.get(0);//from the first leg
    ...
    public void setSpeed(double speed){
        this.speedInMaps = speed;
    }

    public double getSpeed(){
        return this.speedInMaps;
    }
