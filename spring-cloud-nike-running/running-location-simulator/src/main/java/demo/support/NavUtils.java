...
public class NavUtils {
    ...
    public static double getDistance(Point point1, Point point2) {
        ...
        com.spatial4j.core.shape.Point p1 =
                ctx.makePoint(point1.getLongitude(), point1.getLatitude());
        com.spatial4j.core.shape.Point p2 =
                ctx.makePoint(point2.getLongitude(), point2.getLatitude());
        return DistanceUtils
                .degrees2Dist(ctx.getDistCalc().distance(p1, p2),
                        DistanceUtils.EARTH_MEAN_RADIUS_KM) * 1000;
    }
    ...
    4. use NavUtils.getPosition in moveRunningLocation() in task/LocationSimulator.java

    Implement PositionService
    PositionService is designed for sending position info to distribution service.
    1. Create a new package named as service and create PositionService interface.
     * @return
     */
    public static double getBearing(Point pt1, Point pt2) {
        ...
        double x = Math.cos(latitude1) * Math.sin(latitude2) -
                Math.sin(latitude1) * Math.cos(latitude2) * Math
                        .cos(longDiff);
        return (Math.toDegrees(Math.atan2(y, x)) + 360) % 360;
    }
    // getPosition function
        /**
         * Returns coordinates of position which is given distance and bearing
         from given point.
         *
    ...
    public static Point getPosition(Point pt1, double d, double brg) {
        ...
        double lat2 = Math.asin(Math.sin(lat1) * Math.cos(d /
                EARTH_RADIUS_IN_METERS) + Math.cos(lat1) * Math
                .sin(d / EARTH_RADIUS_IN_METERS) *
                Math.cos(brgAsRadians));
        double x = Math.sin(brgAsRadians) * Math.sin(d /
                EARTH_RADIUS_IN_METERS) * Math.cos(lat1);
        double y = Math.cos(d / EARTH_RADIUS_IN_METERS) - Math.sin(lat1) *
                Math.sin(lat2);
    ...
    public static List<Point> decodePolyline(String polyline) {
        ...
        return latLngs.stream().map(latLng -> new Point(latLng.lat,
                latLng.lng))
